    ...
    return (
        <tbody>
            {props.vehicles.map((vehicle, index)=> {
                if(props.userRole.name == authUser) {
                    return (
                    <tr key={vehicle.id}>
                        <td>{vehicle.id}</td>
                        <td>{vehicle.title}</td>
                    ...
                    if(props.userRole.company_id == vehicle.company_id) {
                        return (
                            <tr key={vehicle.id}>
                                <td>{vehicle.id}</td>
                                <td>{vehicle.title}</td>
                                ...
                                <td className="action">
                                    <Link to={'/dashboard/edit-vehicle/'+vehicle.id} vehicleid={vehicle.id}  className="tooltipped" data-position="top" data-delay="50" data-tooltip="Edit"><i className="material-icons">edit</i></Link>
                                    {props.userRole.name == authUser ? (
                                        <a className="tooltip" href="javascript:void(0);" onClick={()=>props.deleteItem(vehicle.id)} data-tooltip="Delete"><i className="material-icons">delete</i></a>
                                    ): null }
                                    {props.confirmText==vehicle.id ? (
